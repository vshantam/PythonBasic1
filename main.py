...
class Monopoly(object):
    ...
    def throw_dices(self):
        return [self.throw_dice() for _ in range(2)]


    def get_new_player_position(self,current_cell, thrown_number, player):
        """
    ...
    def check_number_of_players(self,number_of_players):
        """
    ...
    def get_number_of_players(self):
        ...
        while True:
            ...
            if self.check_number_of_players(number_of_players) == 1:
                return int(number_of_players)

    def input_player_name(self,player, names):
        while True:
            name = raw_input('Player_%d name: ' % (player + 1))
            if name not in names:
                return name
            print "Name {0} exists".format( name)


    def shuffle_players_profiles(self,profiles):
        shuffled_list = profiles[:]
        shuffle(shuffled_list)
        return shuffled_list


    def generate_all_players_names(self,number_of_players):
        names_profiles = []
        for player_number in range(number_of_players):
            players_name = self.input_player_name(
            player_number,
            names_profiles
            )
    ...
    def generate_all_players_profiles(self,names):
        return [self.create_profile(name) for name in names]


    def get_statistics(self,player, player_throw):
        return ' %s %s %s %s' % (
            player[0],
            player_throw,
            player[2],
    ...
    def print_statistics(self,player_throw, player):
        raw_input(self.get_statistics(player, player_throw))


    def update_funds(self,player, old_player_position):
    ...
    def hitting_bonuses_and_taxes_fields(self,player, generated_field):
    ...
    def passing_fields(self,player, generated_field, shuffled_profiles):
        if type(generated_field[player[2]]) == str:
            print player[0], " skip next turn"
            player[3] = True
        else:
            self.hitting_bonuses_and_taxes_fields(player, generated_field)
            self.buying_property(player, generated_field, shuffled_profiles)
        return player


    def buying_choice(self,key, value, player):
        print key, value[1]
    ...
    def buying_property(self,player, generated_field, shuffled_profiles):
        if type(generated_field[player[2]]) == dict:
            for key, value in generated_field[player[2]].iteritems():
                if value[2] is None:
                    self.buying_choice(key, value, player)
    ...
    def main(self):
        ...
        names = self.generate_all_players_names(number_of_players)

        players_profiles = self.generate_all_players_profiles(names)

        shuffled_profiles = self.shuffle_players_profiles(players_profiles)
        ...
        while True:
            ...
            for player in shuffled_profiles:
                ...
                if player[3] is False:
                    ...
                    player_throw = self.throw_dices()

                    old_player_position = player[2]

                    while player_throw[0] == player_throw[1] and player[3] == False:

                        self.get_new_player_position(player[2], sum(player_throw), player)

                        self.update_funds(player, old_player_position)

                        self.print_statistics(player_throw, player)

                        player_throw = self.throw_dices()

                        old_player_position = player[2]
                        self.passing_fields(player, generated_field, shuffled_profiles)
